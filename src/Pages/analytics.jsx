...
function WeeklyGoal({ applicationsThisWeek }) {

    const weeklyGoal = 10;

    const progress = Math.min(
        (applicationsThisWeek.length / weeklyGoal) * 100,
        100
    );

    const remaining = Math.max(
        weeklyGoal - applicationsThisWeek.length,
        0
    );

    const exceeded =
        applicationsThisWeek.length > weeklyGoal;

    function getMessage() {

        if (applicationsThisWeek.length === 0)
            return "Start applying today 🚀";

        if (applicationsThisWeek.length < weeklyGoal)
            return `${remaining} application${remaining !== 1 ? "s" : ""} left this week`;

        if (applicationsThisWeek.length === weeklyGoal)
            return "🎉 Weekly goal achieved!";

        return `🔥 ${applicationsThisWeek.length - weeklyGoal} above your goal`;
    }
    ...
    function getQuote() {
        ...
        if (progress === 0) {
            return {
                ...
                quote: "Every successful career starts with a single application. Take the first step today and begin building momentum."
            };
        }
        ...
        if (progress <= 25) {
            return {
                ...
                quote: "Great start! Consistency matters more than speed. Keep applying regularly and opportunities will follow."
            };
        }
        ...
        if (progress <= 50) {
            return {
                ...
                quote: "You're making solid progress. Stay consistent this week and you'll be one step closer to your career goals."
            };
        }
        ...
        if (progress <= 75) {
            return {
                ...
                quote: "Excellent work! You're close to reaching your weekly target. Keep the momentum going."
            };
        }

        if (progress < 100) {
            return {
                heading: "One Final Push 🎯",
                quote: "You're just a few applications away from completing your goal. Finish strong!"
            };
        }
        ...
        if (applicationsThisWeek.length === weeklyGoal) {
            return {
                ...
                quote: "Congratulations! You've completed your weekly goal. Celebrate your consistency and keep aiming higher."
            };
        }

        return {
            heading: "Hands off 🙌",
            quote: "You've gone beyond your weekly target. Your dedication and consistency are setting you up for more opportunities."
        };
    ...
    return (
        ...
        <div className="weekly-goal-card">
            ...
            <header className="weekly-goal-header">
